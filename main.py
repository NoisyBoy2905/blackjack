...
def blackjack(deck):
    ...
    if dealer_total > 21 or player_total > dealer_total:
        ...
        print(f"Tie! Both have {player_total}")
        return "tie", double
...
def main():
    ...
    while keep_playing:
        ...
        divider()
        print(f"Chips: {chips}  |  Wins: {wins}  Losses: {losses}  Draws: {tie}")
        divider()

        bet = 0
        while bet <= 0 or bet > chips:
            try:
                bet = int(input("How much would you like to bet? ").strip())
            except ValueError:
    ...
    divider()
    print(f"Final: Wins: {wins} | Losses: {losses} | Draws: {tie}")
